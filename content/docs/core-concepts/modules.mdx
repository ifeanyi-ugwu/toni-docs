...
You can attach lifecycle callbacks directly to the module by switching from `pub struct` to `impl` and declaring the hooks inside that block:
...
Module-level hooks are synchronous and run before the provider hooks of the same phase.
