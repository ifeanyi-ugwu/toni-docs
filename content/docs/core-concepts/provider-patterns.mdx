...
#[injectable(pub struct AppService {
    ...
    // You can also inject a type-registered provider by its token
    #[inject("PRIMARY_DB")]
    db: DatabaseService,
})]
impl AppService {
    pub fn new(name: String, port: u16, db: DatabaseService) -> Self {
        Self { name, port, db }
    }
}
```

When no token is specified, `#[inject]` resolves by the field's type name. The `#[inject("TOKEN")]` form resolves by the explicit token string.
