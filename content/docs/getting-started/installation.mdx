...
`ToniFactory::create` is the synchronous-looking entry point; it returns a `ToniApplication<A>` where `A` is the adapter type. No `app.init()` call is needed — initialization runs during `create`.
</Bridge>
