...
4. Registers all routes with the HTTP adapter when `use_http_adapter` is called
5. Runs `on_module_init` hooks on all providers
6. Runs `on_application_bootstrap` hooks

By the time `start()` is called, every service is fully initialized and the route tree is ready.
