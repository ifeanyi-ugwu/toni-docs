...
Implement `ContextBuilder` to populate the context for each request. The `build` method receives the incoming `RequestPart` so you can read headers (e.g. auth tokens) alongside injected services:
