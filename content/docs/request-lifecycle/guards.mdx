...
<Bridge>
Guards in Toni are the direct equivalent of NestJS's `CanActivate` guards. The method is synchronous by default (`can_activate` returns `bool`), same as NestJS's `canActivate` returning a plain boolean. For async token verification you'll implement a service that caches validation results, or return a cached `bool`.
</Bridge>
