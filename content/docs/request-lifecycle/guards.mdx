...
<Bridge>
Guards in Toni are the direct equivalent of NestJS's `CanActivate` guards. `can_activate` is **async** — you can perform JWT verification, database lookups, or any async work directly inside the guard. Returning `true` allows the request; `false` blocks it with a 403.
</Bridge>
...
impl Guard for StrictAuthGuard {
    async fn can_activate(&self, context: &Context) -> bool {
        ...
        if req.headers.get("authorization").is_none() {
            // Returning false yields a 403; throw HttpError for a custom status
        }
        false
    }
}
